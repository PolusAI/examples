...
with st.form(key="my_form"):
    ...
    work_type = st.selectbox(
        "Work Type",
        ["Government", "Never worked", "Private", "Self-employed", "children"],
    )
    ...
    if submit:
        ...
        predict_inputs = predict_inputs.assign(
            hypertension=[inputs.iloc[0]["hypertension"]]
        )
        predict_inputs = predict_inputs.assign(
            heart_disease=[inputs.iloc[0]["heart_disease"]]
        )
        ...
        for cols in cols_lists:
            predict_inputs = predict_inputs.reindex(
                predict_inputs.columns.union(cols, sort=False), axis=1, fill_value=0
            )
